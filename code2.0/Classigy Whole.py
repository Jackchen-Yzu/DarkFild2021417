...
def extract(dark_image_path,low_array,up_array):
 #nparray形式 np.array([156, 43,46]) /////   np.array([180, 255, 255])
 #定义常量
...
model = load_model('E:/cmy/CNN/          .model')
lb = pickle.loads(open('E:/cmy/CNN/   .  ', "rb").read())
...
def load_modellabel(singleimg_list):
    ...
    for k in range(len(singleimg_list)):
        ...
        image = cv2.resize(input_img, (20, 20))
        image = image.astype("float") / 255.0
        image = img_to_array(image)
        image = np.expand_dims(image, axis=0)
        proba = model.predict(image)[0]  # (输入测试数据,输出预测结果)
...
ppp = extract('E:/2.tif',np.array([156, 43,46]),np.array([180, 255, 255]))

pppp = load_modellabel(ppp)


img = cv2.imread('E:/2.tif')
